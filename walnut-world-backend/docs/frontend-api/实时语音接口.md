...
2026-09-09 · 后端协议已实现，按现有按钮进行前端联调。返回 [联调主文档](README.md)。
...
现有适配器参考文档：[实时语音 3.0 全双工接口](https://docs.volcengine.com/docs/6561/2549778?lang=zh)、[接入必读](https://docs.volcengine.com/docs/6561/2549732?lang=zh)。复用现有 JSON WebSocket 适配器，模型 `1.2.6.1`，无需独立 STT 或 TTS。
...
也支持 `YAYA_DOUBAO_VOICE_API_KEY`，与 `_FILE` 二选一。密钥只留后端。默认音色为 `ICL_uranus_zh_male_youmodaye_tob`；音色、语速、音量选项见 [适配器说明](../../../agent/docs/DOUBAO_REALTIME_VOICE.md)。

`walnut-world-backend/scripts/start-persistent-play.ps1` 优先保留显式配置，其次查找 `%USERPROFILE%\.walnut-secrets\doubao-voice.key`，最后兼容旧 `agent/doubao-voice-api.key`。只有找到实时语音凭据且没有显式 mode 时才开启豆包；无配置时默认关闭。显式 `disabled` 始终保留。书书配音独立使用 `.walnut-secrets\book-tts.key`，不会自动把它用于实时对话。`-Action Check` 可在启动前验证配置，实际供应商权限需要单独验证；已有进程的环境不会自动随新终端变化。

## 8. 实现与验收

- [WebSocket 路由](../../src/walnut_backend/api/routes/dingdang_voice.py)
- [数据库上下文](../../src/walnut_backend/adapters/postgres/dingdang_voice.py)
- [主应用装配](../../src/walnut_backend/api/app.py)
- [豆包适配器](../../../agent/python/yaya_agent_runtime/adapters/doubao_realtime.py)
- [离线路由测试](../../tests/unit/test_dingdang_voice.py)
- [PostgreSQL 集成测试](../../tests/integration/test_dingdang_voice_context.py)
...
后端使用 `YAYA_VOICE_MODE=doubao`，并在服务器配置 `YAYA_DOUBAO_VOICE_API_KEY_FILE` 或对应 API Key 环境变量。前端只持有游戏 token；语音服务凭据不写入前端配置。当前适配器使用模型 `1.2.6.1`。

本页根据当前 `api/routes/dingdang_voice.py` 和 Agent 豆包适配器核对；不是要求前端另接一套豆包官方协议。

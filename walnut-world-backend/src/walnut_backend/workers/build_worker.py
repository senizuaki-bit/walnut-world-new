        ...
        async with self._sessions() as session, session.begin():
            ...
            rejected_receipt = await self._jobs.record_step_in_session(
                ...
                output={
                    ...
                    "diagnostic_codes": list(diagnostic_codes),
                    "source_sha256": result.source_sha256,
                    # Keep compiler text in the internal receipt; the public
                    # Evidence schema and failure-count grouping stay unchanged.
                    "diagnostic_messages": [item.message for item in result.failure.diagnostics],
                    "build_identity": result.build_identity,
                },
            )
            rejected = replace(

...
def _rejected_build_matches(
    ...
    if (
        ...
        or set(output) - {"diagnostic_messages"}
        not in (
            {
                "build_id",
                "evidence_id",
        ...
        or (
            "diagnostic_messages" in output
            and (
                not isinstance(output["diagnostic_messages"], list)
                or any(not isinstance(item, str) or not item for item in output["diagnostic_messages"])
            )
        )
        or output.get("build_id") != row.build_id
        or receipt.receipt_id

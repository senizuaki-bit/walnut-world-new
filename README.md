...
| Bug 军团（新Demo协议） | 主关正确后，每局一次变式挑战 | 本局真实错误代码与证据；无错误时巩固 | 只提问而不出题，或一局重复生成 |
| 书书（新Demo协议） | 主关与Bug挑战均通过后 | 主关/挑战代码、判题记录和尝试次数 | 提前结算，或把一次成功说成永久掌握 |
...
5. **教学回流** —— 主动求助调用叮当；新Demo主关成功后完成一次Bug变式挑战，再由书书总结并配音。正式主关脱敏证据继续进入学习档案，练习尚未做持久化归档。

...
> 文档版本：v1.2
> 文档范围：只定义前端、Python 后端、Agent Runtime、游戏运行时与飞书适配器之间的接口和联调规则
> 机器权威：仓库根目录下的 `contracts/`
> 内部 Port 权威：`python/yaya_agent_contracts/ports.py`
> 相关文档集：根目录 `05_核桃代码世界_接口对齐与联调规范.md` 记录五份开发文档的版本治理；本指南面向仓库内接口使用，两者均不高于机器合同
...
当前 01—04 均已修订到 v1.2。下表只记录 `核桃代码世界_技术文档_v1.0.zip` 的**历史基线 hash**，用于识别和审计历史包，不表示当前工作区文件仍等于这些字节，也不参与当前联调验收：
...
合同定义的是目标接口、状态机和验收边界，不等于当前 Adapter 已全部交付。实现状态必须由代码、交付清单和自动化测试证明；不得因为本指南列出了 operation 就宣称“当前已支持”。本指南与根目录 05 的共享接口规则应同步维护；文档集版本、历史 hash、绝对/相对路径等分发元数据可以不同。若两份人类文档的接口语义发生差异，直接回到上述机器权威裁决，不在两份 Markdown 之间制造新的权威层级。
...
A6 历史 E2E 的集中设置包含 Task、初始 World、Session 和两版 Certified Skill/active binding，只证明了运行后角色链。A8 前置 public-chain E2E 只允许集中设置已发布 Task/Content、初始 World、Learner/actor、Agent Profile、Build policy/pinned image 和空 Artifact root；Session、Draft、Build、Artifact、SkillVersion、Certification、Activation 和 Registry 必须由公共 HTTP + durable Worker 创建。它仍只是“已发布内容与初始世界权威后的学生源码公共生产链”，不是空账号旅程。
...
- 合同允许的 LLM/provider 降级必须显式表达 `source`、`degraded` 和 `fallback_reason`，不能假装正常模型结果；但 A6 的 Bug／书书验收只接受 `source=provider`、`degraded=false`，角色/Evidence/phase/Patch/永久掌握等越界在公开写入前 fail closed。
...
### 5.1 当前已装配的 Product 读取

唯一生产入口 `walnut-world-backend` 已装配 ContentUnit、SessionWorkspace、SkillDraft GET/PUT、AgentInteraction list/get与只读INT2 capability。Agent仓的历史HTTP不属于生产拓扑。PatchDecision默认关闭并按Backend flag条件挂载；focused证据不等于正式M2已获准启用。
...
本节描述冻结合同与后续目标，不是 INT1 production 交付声明。Backend 默认不注册 WSS 或 Client Event Batch route，Bootstrap 的 `world_event_stream=false`、`client_event_batch=false` 才是可用性权威。冻结 Schema 仍要求 `stream_url`；默认无凭据 `wss://localhost/v1/realtime` 是 inert 结构值，客户端不得因 URL 存在而连接。
...
首次任务顺序必须固定为 `Bootstrap → Session identity → Draft → Build/Certification → Activation → original Session exact-version Turn`。Product Draft path 需要 canonical `session_id`，所以不能把 `Draft → Build → Activation → Session` 解读为首次创建顺序。Session 创建不要求已激活 Skill。
...
世界提交成功后即使 Provider 失败，Run、World 和 Evidence 仍保留客观结果；客户端不得无限等待或伪造 AI 成功。A6 Bug／书书链会终结 Command 且不发布 provider_fallback Interaction，Product list 保持不变；其他允许 fallback 的通用路径仍必须显式显示降级，不能冒充正常 Provider 结果。

当前正式 Godot AppRoot 已有 offline 60/60、M1 deterministic/restart、完整 M2 deterministic/formal/outage/restart 与受控 real-Provider M2 PASS。学生经公开 UI 请求并确认 Patch 后，分别手动 Build、Activate、Run；Command 11 = 7 `APPLIED` + 4 `REJECTED`，World commit 1、presentation events 8，第二 Godot 进程 recovery-only 为 17 GET / 0 mutation。real-Provider run `868a` 用时 301.012 秒，18 unique dispatch / 18 generation、单 dispatch 最大 generation 1，Patch 状态为 `PUBLIC_UI_CHAIN_CLOSED`。INT1 历史 live 不替代当前 INT2 live；当前结果也不证明 production private DinD 或公开 Gateway pending write response-loss。PatchDecision 默认关闭并条件挂载；WSS、Event Batch、Feishu 与自动/多文件 Patch 排除。
...
每个阶段逐字段核对身份、revision、sequence 和 hash，不只断言 HTTP 2xx。旧 573/252/46 与 169.836 秒 fixture 链均为 historical evidence；当前 Agent discovery 601 = 599 non-live PASS + 2 live opt-in `EXCLUDED_NOT_RUN`、0 skip，Backend full 468/468，Frontend offline 60/60 + 2 real opt-in excluded。完整 M2 deterministic/formal/outage/restart 已以 `DETERMINISTIC_LOCAL_RELAY_ONLY_NOT_REAL_PROVIDER` 分类 PASS；受控 real-Provider run `868a` 也在 301.012 秒取得 PASS，18 unique dispatch / 18 generation、单 dispatch最大 generation 1，`PUBLIC_UI_CHAIN_CLOSED`、World commit 1、presentation events 8、第二进程 17 GET / 0 mutation均闭合。PatchDecision 默认关闭并条件挂载。
...
GitHub Actions CI 使用仓库锁定的 `requirements-ci-win-py312.txt` 和 Windows/Linux x64 wheel hashes。缺验证依赖必须明确失败，不能静默 skip。
...
- [x] 本指南与根目录 05 已同步到 INT2 单 Gateway、默认关闭和证据分级边界；两者都不覆盖机器合同；
- [x] 所有实现以 `contracts/` 为 Wire 权威；
- [ ] 前端没有旧 `/api/*` 调用；
- [ ] 前端不直接调用 LLM、Sandbox、数据库或飞书 SDK；
- [x] Game 202 能通过 Command 和 Location 跨重启对账；
- [x] Product Content/Workspace/Draft/Interaction 已装配；Draft 支持幂等、CAS 与响应不确定对账；
- [x] 没有独立 Run POST，Sandbox 不直接写世界；
- [x] Runtime internal events 与客户端 WSS 已分开；
- [x] WSS 与 Client Event Batch 默认不挂载、Bootstrap capability=false；INT1 只验收 HTTP Events/Snapshot；
...
- [x] production HS256 authority seeder 只创建允许的初始权威；Session/Draft 等业务结果必须由公共链创建；
...
- [x] 全 scope Activation registry CAS、响应丢失、lease takeover、全进程重启和 durable corruption 矩阵均 fail closed 且无重复副作用；
- [x] 194.12秒 INT1 real Provider已归类为 historical host-Docker evidence；当前 INT2 real-Provider authority 为 run `868a` 的 301.012 秒 PASS；
- [x] SkillPatch/PatchDecision仍默认关闭；capability GET始终挂载，mutation route按flag条件挂载；
...
01—04 继续承担产品、架构和实现说明。本指南与根目录 05 共同保持 Agent library、唯一 Backend Gateway/PostgreSQL/migration/workers 和正式 Godot AppRoot 边界。M1 formal deterministic、完整 M2 deterministic/formal/outage/restart 与受控 real-Provider M2 均为 current PASS；production private DinD 与公开 Gateway pending write response-loss 仍为 `NOT_PROVEN`。Skill Patch 是默认关闭、条件启用的当前目标；WSS、Event Batch、Feishu 和自动/多文件 Patch 继续排除。

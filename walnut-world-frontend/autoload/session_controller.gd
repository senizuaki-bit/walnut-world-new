...
##
## The sequence is a parameter rather than something re-derived from Workspace so
## that a corrected retry can reuse this path verbatim. Each attempt mints its own
## turn_id, identity and Idempotency-Key, so a refused attempt leaves nothing
## behind for the next one to collide with.
func _submit_turn_attempt(
	slot: String,
	session: Dictionary,
	world: Dictionary,
	...
	if world_presentation_enabled:
		if world_event_player == null or not world_event_player.has_method("get_cursor"):
			return _local_failure(
				"PRESENTATION_CURSOR_UNAVAILABLE",
				"The authoritative presentation cursor is unavailable before Run submission.",
			)
	...
	if not envelope_result.get("ok", false):
		return envelope_result
	return await _execute_pending_turn_envelope(slot, envelope_result.value, false)


## True when the gateway refused the request itself, so no Turn was created and a
## corrected retry cannot duplicate one. Same statuses that drop the stranded
...
##
## Deliberately not recover_workspace: that also replaces the Draft, and the
## student may have unsaved edits in the editor. Nothing here is written back to
## ClientStore either -- the fresh number is used for one retry and discarded, so
## a refusal can never rewrite local authority.
##
## Returns the corrected client_turn_sequence, or 0 when there is nothing to
## correct, in which case the caller reports the gateway's real refusal unchanged
## instead of hiding it behind a retry.
func _resynced_turn_sequence(session_id: String, attempted_sequence: int) -> int:
	if session_id.is_empty() or product_gateway == null or not product_gateway.has_method("get_workspace"):
		return 0
	var refreshed: Dictionary = await product_gateway.get_workspace(_new_request_context(), session_id)
	if not refreshed.get("ok", false):
		return 0
	var value: Variant = refreshed.get("value")
	if not value is Dictionary:
		return 0
	var refreshed_session: Variant = value.get("session")
	if not refreshed_session is Dictionary:
		return 0
	if str(refreshed_session.get("session_id", "")) != session_id:
		return 0
	var cursor: Variant = refreshed_session.get("last_turn_sequence")
	if typeof(cursor) != TYPE_INT and typeof(cursor) != TYPE_FLOAT:
		return 0
	var corrected := int(cursor) + 1
...
## Guarantee that the transient TURN_RUNNING marker never outlives the operation
## that set it.  The executor raises it before submitting, and every failure path
## (transport failure, terminal Command/Run failure) returned early without
## lowering it, even though TURN_RUNNING is not a state the readiness gate accepts.
##
## Callers that funnel their failure through ClientStore.report_error masked this,
## because report_error overwrites the flow with ERROR anyway.  AppRoot's startup
## recovery does not: it reports startup failure directly, so nothing else would
## ever clear the marker there.
func _execute_pending_turn_envelope(slot: String, envelope: Dictionary, recovery_mode := false) -> Dictionary:
	var store := _client_store()
	var flow_before: int = int(store.flow_state) if store != null else -1
	var result: Dictionary = await _execute_pending_turn_envelope_inner(slot, envelope, recovery_mode)
	if (
		store != null
		and store.flow_state == WalnutClientStore.FlowState.TURN_RUNNING
		and flow_before != WalnutClientStore.FlowState.TURN_RUNNING
	):
		# Restore verbatim, never "upgrade".  ERROR is deliberately fail-closed
		# (submit_and_run_flow_test pins that), so inventing READY here would grant
		# more authority than the client held before the Turn.
		store.set_flow(flow_before)
	return result


func _execute_pending_turn_envelope_inner(slot: String, envelope: Dictionary, recovery_mode := false) -> Dictionary:
	...
	if recovery_mode and not recovery.is_empty():
		...
		var refused: Variant = submission.get("status")
		if (
			not submission.get("ok", false)
			and typeof(refused) == TYPE_INT
			and int(refused) in UNACCEPTED_TURN_HTTP_STATUSES
		):
			# The gateway refused the request itself, so it never became a Turn.
			# Keeping the envelope would make every later student action -- Run
			# included -- reconcile a Turn that can never close, so one refused
			# 问叮当 would permanently block editing and running the code.
			#
...
func _student_action_readiness(action: String) -> Dictionary:
	...
	if (
		...
		or store.flow_state not in STUDENT_ACTION_READY_FLOW_STATES

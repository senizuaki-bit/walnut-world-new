...
extends "res://addons/yaya_contract_client/agent_api_transport.gd"
...
const StrictJsonObjectScanner = preload("res://addons/yaya_contract_client/strict_json_object_scanner.gd")
...
func _build_request_spec(operation: String, arguments: Dictionary) -> Dictionary:
	...
	match operation:
		...
		"get_product_content_unit":
			var content_ref: Variant = arguments.get("content_ref")
			if not content_ref is Dictionary:
				path = "/__invalid_argument__"
			else:
				path = "/product-experience/v1/content-units/%s/versions/%s?content_hash=%s" % [_path_argument(content_ref, "unit_id"), _path_argument(content_ref, "version"), _query_argument(content_ref, "content_hash")]
		"list_product_agent_interactions":
			path = "/product-experience/v1/sessions/%s/agent-interactions?after_sequence=%s&limit=%s" % [
				_path_argument(arguments, "session_id"),
				_query_argument(arguments, "after_sequence"),
				_query_argument(arguments, "limit"),
			]
		"get_product_agent_interaction":
			path = "/product-experience/v1/sessions/%s/agent-interactions/%s" % [
				_path_argument(arguments, "session_id"),
				_path_argument(arguments, "interaction_id"),
			]
		"get_product_session_workspace":
			path = "/product-experience/v1/sessions/%s/workspace" % _path_argument(arguments, "session_id")
		"get_product_skill_draft":
			path = "/product-experience/v1/sessions/%s/skill-drafts/%s" % [
				_path_argument(arguments, "session_id"),
				_path_argument(arguments, "draft_id"),
			]
		"upsert_product_skill_draft":
			...
			body_value = arguments.get("request")
		"record_product_patch_decision":
			method = HTTPClient.METHOD_POST
			path = "/product-experience/v1/sessions/%s/agent-interactions/%s/patches/%s/decision" % [
				_path_argument(arguments, "session_id"),
				_path_argument(arguments, "interaction_id"),
				_path_argument(arguments, "patch_id"),
			]
			body_value = arguments.get("request")
		_:
			return {

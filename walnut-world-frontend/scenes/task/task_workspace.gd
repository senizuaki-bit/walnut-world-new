...
func _on_interactions_recovered(interactions: Array[Dictionary]) -> void:
	...
	if session.has_method("register_visible_patch_failure"):
		...
		_refresh_patch_button()
	var role_names: Dictionary = {"world_agent": "芽芽 / 叮当", "xiaohutao": "小核桃", "teaching_agent": "教学角色", "bug_agent": "Bug 角色", "book_agent": "书书", "system": "系统"}
	...
	dialogue.show_interaction(str(role_names.get(str(latest.get("role", "")), "系统")), str(latest.get("feedback", {}).get("message", "")), "" if question == null else str(question), str(latest.get("response_type", "message")), hint_level)

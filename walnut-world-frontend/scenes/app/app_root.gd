...
@export_group("WATER Candidate Compatibility")
...
@onready var game_flow: Control = get_node_or_null("GameFlow") as Control
@onready var crop_agent_bridge: Node = get_node_or_null("CropAgentBridge")
...
func _configure_crop_agent_bridge() -> void:
	# Script-only configuration tests intentionally instantiate AppRoot without
	# its scene children. The shipped scene must provide the full composition.
	if game_flow == null and crop_agent_bridge == null:
		return
	if game_flow == null or crop_agent_bridge == null or world_event_player == null:
		push_error("AppRoot requires GameFlow, CropAgentBridge, and WorldEventPlayer nodes.")
		return
	var crop_level := game_flow.get_node_or_null("CropAdaptiveWateringDemo") as CropAdaptiveWateringDemo
	if crop_level == null:
		push_error("GameFlow requires the preconfigured CropAdaptiveWateringDemo node.")
		return
	crop_agent_bridge.configure(
		store,
		session_controller,
		crop_level,
		{
			"enabled": water_candidate_compatibility_enabled,
			"content_ref": water_candidate_content_ref.duplicate(true),
			"plot_rules": water_candidate_plot_rules.duplicate(true),
		},
	)
...
func _configure_skill_patch_capability() -> Dictionary:
	...
	if enabled:
		return _local_failure(
			"SKILL_PATCH_UI_UNAVAILABLE",
			"CropAdaptiveWateringDemo Skill Patch decisions are not connected yet.",
		)
...
func _finish(result: Dictionary) -> void:
	...
	if crop_agent_bridge != null and is_instance_valid(crop_agent_bridge):
		if bool(final_result.get("ok", false)):
			if crop_agent_bridge.has_method("activate_initial_projection"):
				var projection_result: Variant = crop_agent_bridge.call("activate_initial_projection")
				if not projection_result is Dictionary or not bool(projection_result.get("ok", false)):
					var projection_message := (
						str(projection_result.get("message", "演示关卡首次权威投影失败。"))
						if projection_result is Dictionary
						else "演示关卡首次权威投影失败。"
					)
					final_result = {
						"ok": false,
						"error_code": "CROP_INITIAL_PROJECTION_FAILED",
						"message": projection_message,
					}
					if store != null:
						store.report_error({
							"scope": "CLIENT_LOCAL",
							"code": "CROP_INITIAL_PROJECTION_FAILED",
							"message": projection_message,
						})
			else:
				final_result = {
					"ok": false,
					"error_code": "CROP_INITIAL_PROJECTION_UNAVAILABLE",
					"message": "演示关卡未提供首次权威投影门禁。",
				}
		elif crop_agent_bridge.has_method("reject_initial_projection"):
			crop_agent_bridge.call(
				"reject_initial_projection",

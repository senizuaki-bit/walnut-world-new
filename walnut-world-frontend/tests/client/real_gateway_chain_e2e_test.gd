...
func _initialize() -> void:
	...
	if (
		crop_agent_bridge == null
		or crop_level == null
		or not crop_agent_bridge.has_signal("build_action_finished")
		or not crop_agent_bridge.has_signal("activation_action_finished")
		or not crop_agent_bridge.has_signal("submit_action_finished")
		or not crop_agent_bridge.has_method("_on_submit_requested")
		or crop_level.get_node_or_null("CodeDrawer/Surface/Margin/Content/Actions/RunButton") == null
	):
		_abort("FORMAL_ACTION_PATH_MISSING", "AppRoot does not expose the CropAdaptiveWateringDemo RunButton -> CropAgentBridge Build/Activation/Submit path.")
		return
	if skill_patch_enabled:
		_abort("CROP_SKILL_PATCH_PATH_NOT_ENABLED", "This CropAdaptiveWatering INT1 acceptance path requires skill_patch=false.")
		return
	var crop_ui_ready: Dictionary = await _open_crop_formal_run_ui(app, crop_level, absolute_deadline)
	if not crop_ui_ready.get("ok", false):
		_abort(str(crop_ui_ready.get("code", "CROP_RUN_UI_UNAVAILABLE")), str(crop_ui_ready.get("message", "Crop Run UI did not become actionable.")))
		return
	...
	var failure_build: Dictionary = {}
	var failure_active: Dictionary = {}
	...
	for failure_index in range(failure_roles.size()):
		...
		if failure_index == 0:
			...
			failure_active = store.active_skill_tuple.duplicate(true)
	...
	var run_submission: Dictionary = await _press_crop_run_action(
		crop_agent_bridge, crop_level, true, absolute_deadline,
	)
	if Time.get_ticks_msec() >= absolute_deadline:
	...
	if not run_submission.get("ok", false) or store.flow_state != WalnutClientStore.FlowState.COMPLETED:
		_abort_store("RUN_CLOSURE_FAILED", "The CropAdaptiveWatering RunButton did not close Build/Activation/Submit as COMPLETED.", store)
		return
	...
	if presentation_enabled and (
		...
		_abort("WORLD_PRESENTATION_FORMAL_PLAYBACK_INVALID", "Formal successful Run did not play exactly eight ordered WATER actions through PLAYING.")
		return
...
func _open_crop_formal_run_ui(
	...
	var enter_button := app.get_node_or_null("GameFlow/GameStartScreen/HeroCard/Margin/Content/Copy/EnterButton") as Button
	if enter_button == null:
		return _failure("CROP_ENTER_BUTTON_MISSING", "GameFlow has no production EnterButton.")
	while enter_button.disabled and Time.get_ticks_msec() < absolute_deadline:
		await process_frame
	if enter_button.disabled:
		return _failure("CROP_ENTER_BUTTON_TIMEOUT", "GameFlow EnterButton did not become actionable.")
	enter_button.pressed.emit()
	while not crop_level.visible and Time.get_ticks_msec() < absolute_deadline:
		await process_frame
	if not crop_level.visible:
		return _failure("CROP_LEVEL_TRANSITION_TIMEOUT", "GameFlow did not reveal CropAdaptiveWateringDemo.")
	var opened: Variant = crop_level.open_formal_run_workspace()
	if not opened is Dictionary or not bool(opened.get("ok", false)):
		return _failure("CROP_RUN_WORKSPACE_REJECTED", str(opened.get("message", "Formal Crop Run workspace did not open.")) if opened is Dictionary else "Formal Crop Run workspace did not open.")
	var run_button := crop_level.get_node_or_null("CodeDrawer/Surface/Margin/Content/Actions/RunButton") as Button
	if run_button == null or run_button.disabled or not run_button.visible:
		return _failure("CROP_RUN_BUTTON_UNAVAILABLE", "The production Crop RunButton is not visible and actionable.")
	return {"ok": true}


func _press_crop_run_action(
	crop_agent_bridge: Node,
	crop_level: CropAdaptiveWateringDemo,
	expect_build_activation: bool,
	absolute_deadline: int,
) -> Dictionary:
	var completion := {"done": false, "result": {}}
	var stages: Array[String] = []
	var stage_results: Dictionary = {}
	if expect_build_activation:
		crop_agent_bridge.build_action_finished.connect(func(result: Dictionary) -> void:
			stages.append("BUILD")
			stage_results["BUILD"] = result.duplicate(true)
		, Object.CONNECT_ONE_SHOT)
		crop_agent_bridge.activation_action_finished.connect(func(result: Dictionary) -> void:
			stages.append("ACTIVATION")
			stage_results["ACTIVATION"] = result.duplicate(true)
		, Object.CONNECT_ONE_SHOT)
	...
	if run_button == null or run_button.disabled:
		return _failure("FORMAL_ACTION_BUTTON_UNAVAILABLE", "CropAdaptiveWateringDemo RunButton is not actionable.")
	...
	if not bool(completion.done):
		return _failure("FORMAL_ACTION_PATH_TIMEOUT", "CropAgentBridge Submit signal did not close before the total deadline.")
	var expected_stages: Array[String] = []
	if expect_build_activation:
		expected_stages.assign(["BUILD", "ACTIVATION", "SUBMIT"])
	else:
		expected_stages.append("SUBMIT")
	if stages != expected_stages:
		return _failure("FORMAL_ACTION_STAGE_ORDER_INVALID", "Crop Run action stages were %s, expected %s." % [stages, expected_stages])
	for stage: String in expected_stages:
		var stage_result: Variant = stage_results.get(stage)
		if not stage_result is Dictionary:
			return _failure("FORMAL_%s_ACTION_FAILED" % stage, "CropAgentBridge %s returned no structured result." % stage)
		if not bool(stage_result.get("ok", false)):
			var reported_stage := str(stage_result.get("stage", "UNSPECIFIED"))
			var reported_code := str(stage_result.get("code", "UNSPECIFIED"))
			var reported_message := str(stage_result.get("message", "No diagnostic message was reported."))
			return _failure(
				"FORMAL_%s_ACTION_FAILED" % stage,
				"CropAgentBridge %s failed with reported_stage=%s code=%s: %s" % [
					stage, reported_stage, reported_code, reported_message,
				],
			)
...
func _execute_failed_objective_turn(
	...
	, Object.CONNECT_ONE_SHOT)
	var submission: Dictionary = await _press_crop_run_action(
		crop_agent_bridge, crop_level, expect_build_activation, absolute_deadline,
	)
	if Time.get_ticks_msec() >= absolute_deadline:
	...
	return {
		"ok": true,
		"build": captured_build.value.duplicate(true),
...
func _verify_formal_ui_projection(
	...
	if (
		crop_level == null
		or crop_level.get_script() == null
		or str(crop_level.get_script().resource_path) != "res://scenes/level_demo/crop_adaptive_watering_demo.gd"
		or crop_agent_bridge == null
		or crop_agent_bridge.get_script() == null
		or str(crop_agent_bridge.get_script().resource_path) != "res://scenes/app/crop_agent_bridge.gd"
	):
		return _failure("FORMAL_CROP_UI_MISSING", "AppRoot does not expose the production CropAdaptiveWateringDemo/CropAgentBridge composition.")
	var editor := crop_level.get_node_or_null("CodeDrawer/Surface/Margin/Content/CodeEditor") as CodeEdit
	var run_button := crop_level.get_node_or_null("CodeDrawer/Surface/Margin/Content/Actions/RunButton") as Button
	...
	var completion_card := crop_level.get_node_or_null("CompletionCard") as Control
	...
	if (
		...
		or phase_strip == null
		or not str(phase_strip.text).contains(str(snapshot.get("world_id", "")))
		or not str(phase_strip.text).contains("revision %d" % int(snapshot.get("revision", -1)))
		or not str(phase_strip.text).contains(str(snapshot.get("state_hash", "")))
		or evidence_body == null
		or not feedback is Dictionary
		or not str(evidence_body.text).contains(str(feedback.get("message", "")))
		or not str(evidence_body.text).contains("Receipt")
		or completion_card == null
		or not completion_card.visible
		or int(crop_level.get("_phase")) != CropAdaptiveWateringDemo.Phase.COMPLETED
		or not projection is Dictionary
		or projection.get("content") != store.content
		or projection.get("snapshot") != snapshot
		or projection.get("interaction") != interaction
		or str(projection.get("source", "")) != store.local_source
	):
		return _failure("CROP_UI_PROJECTION_MISMATCH", "CropAdaptiveWateringDemo does not exactly display canonical Content, Draft, AgentInteraction and Snapshot authority.")
	return {
		"ok": true,
		"value": {
...
func _refresh_public_authority(
	...
	var initial_session_request: Dictionary = initial_bootstrap.session.create_request.duplicate(true)
	var refreshed_session_request: Dictionary = refreshed.session.create_request.duplicate(true)
	initial_session_request.erase("expected_world_revision")
	refreshed_session_request.erase("expected_world_revision")
	if (
		...
		or refreshed_session_request != initial_session_request
	):
	...
	if int(refreshed.session.create_request.expected_world_revision) != int(snapshot.revision):
		return _failure("BOOTSTRAP_SESSION_WORLD_NOT_CURRENT", "Refreshed Session create authority does not target the canonical Snapshot revision.")
...
func _verify_failed_run(run: Dictionary, active: Dictionary, _pre_world: Dictionary) -> Dictionary:
	...
	if (
		...
		or sandbox.action_intents.size() != EXPECTED_WATERING_FAILURE_INTENT_COUNT
		or sandbox.get("failure") != null
	):
		return _failure("FAILURE_SANDBOX_RESULT_INVALID", "Failure-version Sandbox must succeed with the exact five-action watering candidate.")
...
func _verify_failed_evidence(
	...
	for reference in references:
		...
		if (
			...
			or int(payload.get("intent_count", -1)) != EXPECTED_WATERING_FAILURE_INTENT_COUNT
		):
			return _failure("FAILURE_EVIDENCE_MISMATCH", "Rejected SKILL_RUN Evidence does not exactly describe the five-intent watering rejection.")

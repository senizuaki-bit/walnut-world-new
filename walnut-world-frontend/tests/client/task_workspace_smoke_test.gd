...
func _initialize() -> void:
	...
	var bug_interactions: Array[Dictionary] = [{"role": "bug_agent", "response_type": "message", "question": null, "hint_level": null, "feedback": {"message": "这是第三次相同失败，请检查边界条件。", "degraded": false, "evidence_refs": []}}]
	...
	if speaker == null or speaker.text != "Bug 角色" or objective_failure_text == null or objective_failure_text.text != "这是第三次相同失败，请检查边界条件。":

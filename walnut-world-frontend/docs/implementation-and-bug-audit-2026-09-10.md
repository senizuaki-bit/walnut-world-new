...
## 当前生效的处理范围：以前端对齐现有后端为准

根据用户后续约束，仅修复已有后端能力对应的前端接入和纯前端状态问题。下文较早的
“尚未接入功能”是盘点记录，不等于全部进入开发范围。

当前确认保留六项：启动门禁/可恢复重试、重置草稿同步、重复通关卡状态、失焦保留已结束
语音文字、同一回答各通道结束事件处理、恢复已有正式分层提示入口。
其中语音结束顺序问题仅在隔离事件序列复现。正常反馈文案技术化、旧测试预期属于前端清理项。

- 通用 TTS、下一关及新课程内容不纳入本轮，不要求后端新增接口。
- 正式分层提示走现有 Agent Turn/Interaction 链路；实时语音不是该操作的替代品。
- 书书现有反馈可以改进呈现，但不得编造后端未返回的学习记录、星级或解锁状态。
- 不能将 Patch 入口隐藏直接判为 Bug：后端 `/product-experience/v1/capabilities` 规定
  开关、失败证据、L4、显式请求、CAS 和学生确认；当前 AppRoot 还同时依赖本地
  `skill_patch_enabled` 与 `world_presentation_enabled`。先满足现有合同与开放条件，
  不为显示按钮而绕过门控，不为此修改后端。

### 修复交付（同日后续授权）

用户已授权只改前端、提交并推送。上述六项现已实现：

- 启动中禁止进入农场；失败显示重连；配置缺失显示未就绪。重连复用权威恢复流程，保留草稿和待协调操作。
- 重置代码主动进入草稿变更链路，重玩不再恢复重置前的代码。
- 正式通关重新设置卡片标题、正文与按钮，下一关预告不再污染后续通关。
- 失焦停止采集和播放，保留已显示文字；同一回答音频结束后的文本不会清空或重复开启回答。
- 恢复农场与预置编辑器内的「给我提示」按钮，调用现有 Hint Turn；防重复请求及并发运行，失败恢复按钮并说明如何重试。
- 学生完成正文改为结果说明；精确运行标识放入诊断提示。修正旧测试对 PROVIDER_UNAVAILABLE 的预期。

验证：Godot 4.7.1 离线套件执行 86 项，首次 85 通过、1 项新增提示失败文案检查失败。
修复后该项及候选提示、反馈队列三个相关测试复跑通过，无未解决测试失败。
新增真实回环 HTTP 用例验证断开→重连→进入、重复点击拦截，恢复现有会话全程仅 GET。
编辑器 1280×720 渲染检查通过，重置/给我提示/直接运行三个按钮均完整可见。
真实本地服务恢复检查通过：更新前后的 Session、Skill、Draft、World、Interaction 游标完全一致，HTTP 写入为零。
联调工作树后端与 origin/main 比较无差异；本轮仅同步前端提交，未推送本地联调分支。
部分原有 headless 用例仍报告退出时 ObjectDB/resource 未释放警告，本轮未处理资源回收问题。
语音验证使用事件序列和 UI 回归，不代表已重新实测麦克风与云端音频链路。

以下内容保留为修复前证据，不代表这些缺陷仍然存在。后端接口、配置与 Agent 本轮均未修改。

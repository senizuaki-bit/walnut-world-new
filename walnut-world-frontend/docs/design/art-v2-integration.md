...
## 二次复核修复与动态偏好（2026-09-06）

- 用户明确不需要“减少动态”：移除 HUD 开关及其专用脚本，不再把对话补间减少动态适配列入待办。素材加载/隐藏时的海报回退保持可用。
- 对话退场中途立即结束时，先取消原退场 Tween；重复结束调用不再再次发出 `sequence_finished`，恢复原焦点与位置。新增退场中断、重复中断、新对话不受旧回调影响的回归。
- 工坊六个输入框在程序清空后再刷新校验和皮肤，避免空内容保留 `filled` 外观。
- 成功/错误图集现在驱动原生检查按钮的 StyleBoxTexture，文字仍由 Button 绘制。用各片段全部帧的 alpha 范围排除透明留白，反馈状态使用浅色文字；结束或离开工坊后恢复普通皮肤。移除右上角的独立反馈色条以及检查按钮整块缩放。
- 预置固定高度的错误提示槽，显示错误不会再向下挤动检查按钮。原校验、立即推进教学步骤及点击处理时序保留。
- 验证：本轮 79/79 离线回归通过；最终按钮显示区域与文字颜色调整后，美术、作物教学、Agent 展示队列三项复测通过。五张实际渲染截图与测试摘要位于 `verification/art-v2-second-fixes/`。真实网关和平台导出未执行。

...
`all` 的正式入口已接入每局一次的 Bug 挑战：主关成功后生成变式题，在独立编辑器中判题，通过后获取书书总结及完整配音。旧自动 Bug/Book 交互仍同步游标，但不再展示。网络恢复、验证结果与后端配音配置见[联调记录](docs/testing/bug-practice-integration.md)。下述旧角色提示行为保留于未启用新练习的兼容入口。

...
func present_agent_world_cue(presentation_key: StringName, active: bool) -> bool:
	if presentation_key != &"bug_legion":
		return true
	var bug_legion := farm_world.get_node_or_null("AgentPresentation/BugLegion")
	if bug_legion == null:
		return false
	if active:
		bug_legion.show_challenge()
	else:
		bug_legion.dismiss()
	return true

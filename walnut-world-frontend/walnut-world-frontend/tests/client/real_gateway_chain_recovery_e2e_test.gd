...
func _initialize() -> void:
	...
	if skill_patch_enabled:
		_abort("CROP_SKILL_PATCH_PATH_NOT_ENABLED", "CropAdaptiveWatering recovery acceptance requires skill_patch=false.")
		return
	if (
...
func _verify_formal_ui_projection(app: Node, store: WalnutClientStore, interaction: Dictionary) -> Dictionary:
	...
	if (
		...
		return _failure("FORMAL_CROP_UI_MISSING", "AppRoot does not expose the recovered CropAdaptiveWateringDemo/CropAgentBridge composition.")
